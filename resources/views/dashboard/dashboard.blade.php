                            ...
                            @foreach ($pesanan as $dtf)
                                    ...
                                    @php
                                        ...
                                        if ($bagian == 'body') {
                                            ...
                                            $daharga = DB::table('tipe_kain_prices')
                                                ->where([['tipe_kain_id', $tipe_kain_id], ['tipe', $satuan_harga], ['customer_category_id', $customer_cat], ['periode', $maxperiode]])
                                                ->first();
                                        
                                            $harga = $daharga->harga ?? 0;
                                        } else {
                                ...
                                <article>
                                    <header class="uk-flex uk-flex-between">
                                        ...
                                        <span
                                            class="uk-label uk-label-{{ $status_warna }}">{{ $text_status_pesanan }}</span>
                                    </header>
                                    <main class="uk-grid-small" uk-grid>
                                        ...
                                        <div class="uk-width-2-5@s">
                                            <dl>
                                                <dt>Catatan</dt>
                                                <dd>{{ $dtf->catatan_cs ?? '-' }}
                                                    {{ $dtf->updated_by != 0 ? ' - Admin' : '' }}</dd>
                                            </dl>
                                        </div>
                                    </main>
                                    <footer class="uk-text-right">
                                        @if ($dtf->status_pesanan_id == 1 || $dtf->status_pesanan_id == 2)
                                            <a href="/draft-so/{{ $dtf->id }}">View<i
                                                    class="ph-light ph-caret-circle-right"></i></a>
                                        @elseif($dtf->status_pesanan_id == 4)
                                            <a href="/detail-so/{{ $dtf->id }}"
                                                class="rz-button uk-button-small">Lacak Pengiriman</a>
                                        @else
                                            <a href="/detail-so/{{ $dtf->id }}">View<i
                                                    class="ph-light ph-caret-circle-right"></i></a>
                                        @endif
                                    </footer>
                                </article>
    ...
    @if (session('success_pesan'))
        <div id="modalSukses" class="uk-flex-top" uk-modal>
            <div class="uk-modal-dialog uk-modal-body uk-margin-auto-vertical uk-text-center">
                ...
                <p>Terimakasih. Pesanan Anda sudah terkirim. Kami akan memeriksa ketersediaan barang tersebut, dan akan
                    menyampaikan informasi kepada Anda.</p>
                ...
                $(document).on('click', '.add-modal-cart', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (info === "RIB") {
                                $('#info-asesoris').text(
                                    "Pembelian rib min. 5% dari pembelian body")
                            } else if (info === "BUR") {
                                $('#info-asesoris').text(
                                    "Pembelian bur min. 20% dari pembelian body")
                            } else if (info === "KER") {
                                $('#info-asesoris').text(
                                    "Pembelian Kerah dan Manset min. 22% dari pembelian body")
                            } else {
                                $('#info-asesoris').text("")
                            }
